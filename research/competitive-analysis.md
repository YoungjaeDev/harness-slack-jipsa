...
|------|-------------|-------------|----------------------|----------------|----------------------|---------------------|------------------------|---------------|-------|
| **mpociot/claude-code-slack-bot** | Per-thread (Slack thread_ts = session key), lifetime = thread open; no expiry | Not observed in reviewed files | `--dangerously-skip-permissions` delegated to local Claude Code process; no shell ACL layer observed in reviewed files | Single daemon; no project isolation concept | None (no Notion/Linear) | Socket Mode; `app_mention` + `message.im` | None — manual `npm start`, tmux suggestion | No test files visible | 159 stars; TypeScript; most popular dedicated Claude Code Slack bot |
...
| **tomeraitz/claude-slack-bridge** | N/A — MCP server model: session is the caller's Claude Code session, not managed by bridge | N/A — bridge is stateless between invocations | MCP `ask_human` tool; Claude Code calls the bridge mid-task; no persistent shell exposure observed in reviewed files | N/A | None | Not a traditional bot; Claude Code calls MCP server which posts to Slack and blocks waiting for reply | N/A — runs as MCP stdio subprocess inside Claude Code | Not visible | 26 stars; Python; fundamentally different interaction model (human-in-the-loop MCP, not autonomous daemon) |
